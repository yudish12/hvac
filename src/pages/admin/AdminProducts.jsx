...
import { supabase, getPublicImageUrl, getPublicBrochureUrl } from '../../lib/supabase'
import {
  Plus, Pencil, Trash2, X, Upload, Search, Image as ImageIcon, Layers, FileText, Download,
} from 'lucide-react'
import {
  productCategories,
  getCategoryLabel,
...
export default function AdminProducts() {
  ...
  function openEdit(product) {
    ...
    setBrochurePreview(product.brochure_url || '')
    setShowModal(true)
  }
  ...
  function handleBrochureSelect(e) {
    const file = e.target.files?.[0]
    if (!file) return
  ...
  async function uploadBrochure(file) {
    ...
    if (error) throw error
    return getPublicBrochureUrl(fileName)
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      let brochureUrl = form.brochure_url
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-600">Brochure</th>
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {/* Brochure Upload */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">Product Brochure (PDF)</label>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <label className="inline-flex items-center gap-2 bg-white hover:bg-slate-100 text-slate-700 px-4 py-2 rounded-xl text-sm font-medium cursor-pointer transition-colors border border-slate-200">
                      <Upload className="w-4 h-4" />
                      Upload PDF
                      <input type="file" accept="application/pdf" onChange={handleBrochureSelect} className="hidden" />
                    </label>
                    {brochurePreview && (
                      <a
                  ...
                  <p className="text-[11px] text-slate-400 mt-2">Or paste a public PDF URL below:</p>
                  <input
                    name="brochure_url"
                    value={form.brochure_url || ''}
                    onChange={(e) => {
                      handleChange(e)
                      setBrochurePreview(e.target.value)
                    }}
                    className="w-full mt-1 px-3 py-2 rounded-lg border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500"
